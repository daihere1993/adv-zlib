...
export interface RefactoredAdvZlibOptions {
  logger?: Logger;
  ...
  maxContentCacheFileSizeMB?: number; // New: Max file size to cache in memory
  tempDir?: string; // New: Directory for disk cache
}
...
export class ZipEntry {
  ...
  public async extract(dest: string): Promise<string> {
    ...
    try {
      ...
      // Use relPath instead of name to preserve directory structure
      dest = stats.isDirectory() ? path.join(dest, this.relPath) : dest;
    } catch (error) {
...
export class RefactoredAdvZlib {
  private logger: Logger;
  private cache: CentralDirCache;
  private contentCache: DecompressedContentCache | null;

  constructor(opts?: RefactoredAdvZlibOptions) {
  ...
  public async getEntries(src: string, filterFn?: (entry: ZipEntry) => boolean): Promise<ZipEntry[]> {
    this.logger.debug(`[RefactoredAdvZlib] getEntries(): Getting entries for ${src}`);

    if (!src) {
      throw new Error(`[RefactoredAdvZlib] getEntries(): src is required`);
    }

    if (!src.endsWith('.zip')) {
      throw new Error(`[RefactoredAdvZlib] getEntries(): ${src} is not a zip file`);
    }
    ...
    if (!centralDir) {
      ...
      if (!(await this.fileOrDirExists(src))) {
        throw new Error(`[RefactoredAdvZlib] getEntries(): File does not exist: ${src}`);
      }
      ...
      this.logger.error(`[RefactoredAdvZlib] getEntries(): Failed to get central directory for ${src}`);
      return [];
    }
  ...
  public async exists(src: string): Promise<boolean> {
    this.logger.debug(`[RefactoredAdvZlib] exists(): Checking if ${src} exists`);

    if (!src) {
      throw new Error(`[RefactoredAdvZlib] exists(): ${src} is empty`);
    }
    ...
    if (!centralDir) {
      this.logger.error(`[RefactoredAdvZlib] exists(): Failed to get central directory for ${src}`);
      return false;
    }
  ...
  public async extract(src: string, dest: string, filterFn?: (entry: ZipEntry) => boolean): Promise<string[]> {
    this.logger.debug(`[RefactoredAdvZlib] extract(): Extracting ${src} to ${dest}`);

    if (!(await this.exists(src))) {
      throw new Error(`[RefactoredAdvZlib] extract(): ZIP file ${src} does not exist.`);
    }

    if (src.endsWith('.zip') && !(await this.fileOrDirExists(dest))) {
      throw new Error(`[RefactoredAdvZlib] extract(): As ${src} is a zip file, ${dest} must be an existing directory.`);
    }

    if (!src.endsWith('.zip') && !(await this.fileOrDirExists(path.dirname(dest)))) {
      throw new Error(`[RefactoredAdvZlib] extract(): ${path.dirname(dest)} does not exist.`);
    }

    if (!src.endsWith('.zip') && filterFn) {
      throw new Error(`[RefactoredAdvZlib] extract(): Filter function is only applicable for extracting a whole zip file.`);
    }

    const centralDir = await this.createOrGetCentralDir(src);
    if (!centralDir) {
      throw new Error(`[RefactoredAdvZlib] extract(): Failed to get central directory for ${src}`);
    }
  ...
  public async read(src: string, filterFn?: (entry: ZipEntry) => boolean): Promise<Buffer> {
    this.logger.info(`[RefactoredAdvZlib] read(): Reading content from ${src}`);
    ...
    const centralDir = await this.createOrGetCentralDir(src);
    if (!centralDir) {
      throw new Error(`[RefactoredAdvZlib] read(): Failed to get central directory for ${src}`);
    }

    const entries = this.getRelatedEntries(src, centralDir.entries, filterFn);
    if (entries.length === 0) {
      this.logger.error(`[RefactoredAdvZlib] read(): No matching entries found in ${src}`);
      return Buffer.alloc(0);
    }

    if (entries.length > 1) {
      this.logger.error(`[RefactoredAdvZlib] read(): Multiple matching entries found in ${src}`);
      return Buffer.alloc(0);
    }
  ...
  public async cleanup(): Promise<void> {
    ...
    this.logger.debug(
      `[RefactoredAdvZlib] cleanup(): Clearing caches - CentralDir: ${centralStats.entries} entries, ${centralStats.memoryMB}MB; Content: ${contentStats.entries} entries (${contentStats.memoryEntries} memory, ${contentStats.diskEntries} disk), ${contentStats.memoryMB}MB`
    );
  ...
  private getLastEntryRelPath(src: string, includeZip = false): string {
    ...
    if (entryRelPath.endsWith('.zip') && !includeZip) {
      return '';
    }
  ...
  private async createOrGetCentralDir(source: string): Promise<CentralDir | null> {
    ...
    if (cachedCentralDir) {
      this.logger.debug(`[RefactoredAdvZlib] createOrGetCentralDir(): Using cached CentralDir for ${lastZipPath}`);
    ...
    if (isDirectZip) {
      ...
      try {
        ...
        try {
          ...
          this.logger.debug(`[RefactoredAdvZlib] createOrGetCentralDir(): Failed to get mtime for ${lastZipPath}: ${error}`);
        }
        ...
        this.logger.debug(`[RefactoredAdvZlib] createOrGetCentralDir(): Failed to open ${lastZipPath}: ${error}`);
        return null;
      }
    }
      ...
      if (segment.endsWith('.zip')) {
        if (!currentCentralDir) {
          ...
          try {
            ...
            this.logger.debug(`[RefactoredAdvZlib] createOrGetCentralDir(): Failed to get mtime for ${segment}: ${error}`);
          }
          ...
          const entryName = path.basename(segment);
          const zipEntry = currentCentralDir.entries.find((entry) => entry.relPath === entryName);

          if (!zipEntry) {
            throw new Error(`ZIP entry not found: ${entryName} in ${currentCentralDir.fullPath}`);
          }
